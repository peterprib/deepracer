...
class Reward:
    ...
    def reward_function(self, params):
        ...
        ##distance_reward = 1/(1 + distance_race_line**2)
        ...
        ##speed_reward = 1/(1+abs(speed_diff)**2)
        ...
        ##heading_reward=1/(1+abs(heading_diff)**2)
        
        reward_calculated = distance_reward * 0.1 + 0.9*(speed_reward * heading_reward)
        ##reward_calculated = distance_reward * 0.1 + speed_reward *0.6 + heading_reward*0.3
        ...
        if self.verbose == True :
            ##print(f"SIMPRIB: {steps:s},{closest_index:d},{next_point:d},{last_point:d},{race_line_closest[0]:f},{race_line_closest[1]:f},{distance_race_line:f},{self.last_distance_race_line:f},{distance_race_line_change:f},{distance_reward:f},{speed_optimal:f},{speed_diff:f},{self.last_speed:f},{self.last_speed_optimal:f},{speed_change:f},{speed_reward:f},{heading},{heading_optimal},{steering_angle},{heading_diff},{self.last_heading:f},{self.last_heading_optimal:f},{heading_change:f},{heading_reward:s},{reward_calculated:f}")

            print(f"Steps: {steps:f}, Points closest: {closest_index:d} next: {next_point:d} last: {last_point:d}")
